...
class SiteTreeMisdirectionExtension extends Extension
{
    ...
    public function validate(ValidationResult $result)
    {
        ...
        $vanityURL = (!Controller::curr() || is_null($controller = Controller::curr()) || is_null($URL = $controller->getRequest()->postVar('VanityURL'))) ? $this->owner->VanityMapping()->MappedLink : $URL;
        if (!$vanityURL) {
            return $result;
        }
        ...
        if ($result->isValid() && $existing && ($page = $existing->getRedirectPage())) {
            ...
            $result->addError("Vanity URL <a href='{$link}' target='_blank'>already exists</a>!", ValidationResult::TYPE_ERROR, '', ValidationResult::CAST_HTML);
        }
    ...
    protected function onBeforeWrite($dummy)
    {
        // Retrieve the vanity mapping URL, where this is only possible using the POST variable.
    ...
    protected function onAfterWrite($dummy)
    {
